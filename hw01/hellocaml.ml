...
(* GOAL: get up to speed with OCaml; write a simple arithmetic
   ...
   The projects for COS320 will be implemented in OCaml.  You won't
   need to understand the entire language for the purposes of COS320,
   but it is essential for the later projects that you have a good
   understanding of the basic features of the language.

   If you want to learn more about OCaml, COS326 will go into far
   greater depth than we will in COS320.  If you've already taken
   COS326, then great -- HW1 will be a breeze (but you should expect
   things to pick up in the remainder of the assignments!)

   A good reference for learning about OCaml is "Real World OCaml" by
   Yaron Minsky, Anil Madhavapeddy, and Jason Hickey (available online
   at https://v1.realworldocaml.org/v1/en/html/index.html).  For the
   purposes of this class, chapters 1-9 are sufficient.  If you have
   never seen OCaml before, it would be a good idea to at least read
   chapter 1 before attempting the assignment.  Problems marked with
   "See RWO Ch 4" indicate the relevant chapter of Real World OCaml
...
(*
  ...
  pattern matching using the 'match-with' notation.
...
(*
  OCaml has a build-in datatype of generic lists: See RWO Ch 3
...
(*
  ...
  Read RWO Ch 3, TAIL RECURSION, and implement a tail recursive version of
...
(* Problem 3-5
   ...
   Implement insert, a function that, given an element x and a sorted
   list l (i.e. one for which is_sorted returns true) returns the list
   obtained by inserting x into the list l at the proper location.
   Note that if x is already in the list then insert should just
   return the original list.

   You will need to use the if-then-else expression. Remember that
...
(* TERMINOLOGY: "Object" level vs. "Meta" level
   ...
   course OCaml will usually be the "meta language".
...
(*
  ...
  See RWO Ch 7 for more about error handling.
