...
let helloworld =
  [ text "foo" [(Xorq, [~%Rax; ~%Rax]); (Movq, [~$100; ~%Rax]); (Retq, [])]
  ; text "main" [(Xorq, [~%Rax; ~%Rax]); (Movq, [Ind1 (Lbl "baz"); ~%Rax]); (Retq, [])]
  ; data "baz" [Quad (Lit 99L); Asciz "Hello, world!"] ]

let factorial_iter n =
  [ text "main" [(Movq, [~$1; ~%Rax]); (Movq, [~$n; ~%Rdi])]
  ; text "loop"
      [ (Cmpq, [~$0; ~%Rdi])
      ; (J Eq, [~$$"exit"])
      ; (Imulq, [~%Rdi; ~%Rax])
...
let factorial_rec n =
  [ text "fac"
      ...
      ; (Addq, [~$8; ~%Rsp])
      ; (Retq, []) ]
  ; text "exit" [(Movq, [~$1; ~%Rax]); (Addq, [~$8; ~%Rsp]); (Retq, [])]
  ; gtext "main" [(Movq, [~$n; ~%Rdi]); (Callq, [~$$"fac"]); (Retq, [])] ]

(* Object Builders *)

let sbyte_list (a : sbyte array) (start : int) : sbyte list = Array.to_list (Array.sub a start 8)

let stack_offset (i : quad) : operand = Ind3 (Lit i, Rsp)

let test_exec : exec =
  {entry= 0x400008L; text_pos= 0x400000L; data_pos= 0x400064L; text_seg= []; data_seg= []}
...
let helloworld_textseg =
  ...
  ; InsFrag
  ; InsFrag
  ; InsB0 (Movq, [Imm (Lit 100L); Reg Rax])
  ; InsFrag
  ; InsFrag
  ; InsFrag
  ; InsFrag
  ...
  ; InsFrag
  ; InsFrag
  ; InsB0 (Movq, [Ind1 (Lit 0x400030L); Reg Rax])
  ; InsFrag
  ; InsFrag
  ; InsFrag
  ; InsFrag
...
let interp_cnd_test (fo, fs, fz) tru () =
  ...
  List.iter
    (fun c ->
      if not (List.mem c tru) then
        failwith (Printf.sprintf "o:%b s:%b f:%b %s expected" fo fs fz (string_of_cnd c))
      else () )
    tru' ;
  List.iter
    (fun c ->
      if not (List.mem c fls) then
        failwith (Printf.sprintf "o:%b s:%b f:%b %s !expected" fo fs fz (string_of_cnd c))
      else () )
    fls'
...
let cs_test (n : int) (m : mach) (fo', fs', fz') =
  cc_test (Printf.sprintf "expected OF:%b SF:%b ZF:%b" fo' fs' fz') n m (not fo', not fs', not fz')
    (fun m -> m.flags.fo = fo' && m.flags.fs = fs' && m.flags.fz = fz')

let cso_test (n : int) (m : mach) (fo' : bool) =
  cc_test (Printf.sprintf "expected OF:%b" fo') n m (not fo', false, false) (fun m ->
      m.flags.fo = fo' )

let csi_test (n : int) (m : mach) =
  cc_test "expected TTT ccodes" n m (true, true, true) (fun m ->
...
let program_test (p : prog) (ans : int64) () =
  ...
  if res <> ans then failwith (Printf.sprintf "Expected %Ld but got %Ld" ans res) else ()
...
let add =
  test_machine
    ...
    ; InsFrag
    ; InsFrag
    ; InsB0 (Addq, [~%Rbx; stack_offset 0L])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
...
let functionality_tests =
  [ ("mov_ri", machine_test "rax=42" 1 mov_ri (fun m -> m.regs.(rind Rax) = 42L))
  ; ( "add"
    , machine_test "rax=rbx=*66528=1" 3 add (fun m ->
          m.regs.(rind Rax) = 1L
          && m.regs.(rind Rbx) = 1L
...
let mov_mr =
  test_machine
    ...
    ; InsFrag
    ; InsFrag
    ; InsB0 (Movq, [~%Rax; stack_offset (-8L)])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
...
let subq =
  test_machine
    ...
    ; InsFrag
    ; InsFrag
    ; InsB0 (Subq, [~%Rbx; stack_offset 0L])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
...
let andq =
  test_machine
    ...
    ; InsFrag
    ; InsFrag
    ; InsB0 (Andq, [stack_offset 0L; ~%Rcx])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
...
let negq =
  test_machine
    ...
    ; InsFrag
    ; InsFrag
    ; InsB0 (Movq, [Imm (Lit Int64.min_int); ~%Rbx])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
...
let shl =
  test_machine
    ...
    ; InsFrag
    ; InsFrag
    ; InsB0 (Shlq, [~%Rcx; stack_offset 0L])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
...
let cmpq =
  test_machine
    ...
    ; InsFrag
    ; InsFrag
    ; InsB0 (Cmpq, [~%Rbx; stack_offset 0L])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
...
let instruction_tests =
  [ ( "mov_mr"
    , machine_test "*65520=42" 2 mov_mr (fun m ->
          int64_of_sbytes (sbyte_list m.mem (mem_size - 16)) = 42L ) )
  ; ( "subq"
    , machine_test "rax=*65528=-1L; rbx=1" 3 subq (fun m ->
          m.regs.(rind Rax) = Int64.neg 1L
          && m.regs.(rind Rbx) = 1L
          && int64_of_sbytes (sbyte_list m.mem (mem_size - 8)) = Int64.neg 1L ) )
  ; ( "andq"
    , machine_test "rax=2 rbx=2 rcx=1 *65528=1" 8 andq (fun m ->
          m.regs.(rind Rax) = 2L
          && m.regs.(rind Rbx) = 2L
          && m.regs.(rind Rcx) = 1L
          && int64_of_sbytes (sbyte_list m.mem (mem_size - 8)) = 1L ) )
  ; ( "negq"
    , machine_test "rax=-42 rbx=min_int64 *65528=24" 6 negq (fun m ->
          m.regs.(rind Rax) = Int64.neg 42L
          && m.regs.(rind Rbx) = Int64.min_int
          && int64_of_sbytes (sbyte_list m.mem (mem_size - 8)) = 24L ) )
  ; ( "shl"
    , machine_test "rax=4 *65528=16" 5 shl (fun m ->
          m.regs.(rind Rax) = 4L && int64_of_sbytes (sbyte_list m.mem (mem_size - 8)) = 16L ) )
  ; ("imul", machine_test "rax=44 *65528=2" 3 imul (fun m -> m.regs.(rind Rax) = 44L))
  ; ( "pushq"
    , machine_test "rsp=4 *65520=2A" 1 pushq (fun m ->
          m.regs.(rind Rsp) = 0x0040FFF0L
          && int64_of_sbytes (sbyte_list m.mem (mem_size - 16)) = 0x2AL ) )
  ; ( "popq"
    , machine_test "rsp=4259832 rax=2A" 3 popq (fun m ->
          m.regs.(rind Rax) = 0x2AL && m.regs.(rind Rsp) = 0x0040FFF8L ) )
  ; ( "cmpq"
    , machine_test "rax=4 rbx=0" 5 cmpq (fun m ->
          m.regs.(rind Rax) = 4L
          && m.regs.(rind Rbx) = 0L
...
let cc_add_1 =
  test_machine
    [ InsB0 (Movq, [Imm (Lit 0xFFFFFFFFFFFFFFFFL); ~%Rax])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
...
let cc_add_2 =
  test_machine
    ...
    ; InsFrag
    ; InsFrag
    ; InsB0 (Addq, [Imm (Lit 0xFFFFFFFFFFFFFFFFL); ~%Rax])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
...
let cc_add_3 =
  test_machine
    [ InsB0 (Movq, [Imm (Lit 0x7FFFFFFFFFFFFFFFL); ~%Rax])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
...
let cc_add_4 =
  test_machine
    [ InsB0 (Movq, [Imm (Lit 0x9000000000000000L); ~%Rax])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ...
    ; InsFrag
    ; InsFrag
    ; InsB0 (Addq, [Imm (Lit 0xA000000000000000L); ~%Rax])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
...
let cc_cmp_1 =
  test_machine
    ...
    ; InsFrag
    ; InsFrag
    ; InsB0 (Cmpq, [Imm (Lit 0x8000000000000000L); ~%Rax])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
...
let cc_cmp_2 =
  test_machine
    [ InsB0 (Movq, [Imm (Lit 0x8000000000000000L); ~%Rax])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
...
let cc_and =
  test_machine
    [ InsB0 (Movq, [Imm (Lit 0x0F0F0F0FL); ~%Rax])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ...
    ; InsFrag
    ; InsFrag
    ; InsB0 (Andq, [Imm (Lit 0xF0F0F0F0L); ~%Rax])
    ; InsFrag
    ; InsFrag
    ; InsFrag
    ; InsFrag
...
let cc_push =
  test_machine [InsB0 (Pushq, [~$0]); InsFrag; InsFrag; InsFrag; InsFrag; InsFrag; InsFrag; InsFrag]
...
let cc_ret =
  test_machine [InsB0 (Retq, []); InsFrag; InsFrag; InsFrag; InsFrag; InsFrag; InsFrag; InsFrag]
...
let easy_tests : suite =
  ...
  ; GradedTest
      ...
      , 5
      , [ ( "load_flags"
          , assert_eqf (fun () -> (load test_exec).flags) {fo= false; fs= false; fz= false} )
        ; ("load_rip", assert_eqf (fun () -> (load test_exec).regs.(rind Rip)) 0x400008L)
        ; ("load_rsp", assert_eqf (fun () -> (load test_exec).regs.(rind Rsp)) 0x40FFF8L) ] ) ]

let medium_tests : suite =
  [ GradedTest
      ( "Medium Assemble Tests"
      , 5
...
let hard_tests : suite =
  [ GradedTest ("End-to-end Factorial", 10, [("fact6", program_test (factorial_rec 6) 720L)])
...
let graded_tests : suite = timeout_suite 1 (easy_tests @ medium_tests @ hard_tests @ manual_tests)
