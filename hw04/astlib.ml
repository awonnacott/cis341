...
let rec print_list_aux fmt sep pp l =
  ...
  | [h] ->
      pp fmt h
  | h :: tl ->
      pp fmt h ; sep () ; print_list_aux fmt sep pp tl

let rec print_ty_aux fmt t =
  let pps = pp_print_string fmt in
  match t with TBool -> pps "bool" | TInt -> pps "int" | TRef r -> print_rty_aux fmt r
...
and print_exp_aux level fmt e =
  ...
  ( match e.elt with
  | CNull t ->
      print_ty_aux fmt t ; pps "null"
  | CBool v ->
      pps (if v then "true" else "false")
  | CInt v ->
      pps (Int64.to_string v)
  | CStr v ->
      pps (Printf.sprintf "%S" v)
  | CArr (ty, vs) ->
      pps "new " ;
      print_ty_aux fmt ty ;
      pps "[]" ;
  ...
  | Id id ->
      print_id_aux fmt id
  | Index (e, i) ->
      print_exp_aux this_level fmt e ; pps "[" ; print_exp_aux 0 fmt i ; pps "]"
  | Call (e, es) ->
      print_exp_aux this_level fmt e ; print_exps_aux "(" ")" fmt es
  | NewArr (ty, e1) ->
      pps "new " ; print_ty_aux fmt ty ; pps "[" ; print_exp_aux 0 fmt e1 ; pps "]"
  | Bop (o, l, r) ->
      pp_open_box fmt 0 ;
...
and print_cond_aux fmt b_then opt_b_else =
  ...
  match opt_b_else with [] -> () | b_else -> pps " else " ; print_block_aux fmt b_else
...
and print_stmt_aux fmt s =
  ...
  | Ret eo ->
      pps "return" ;
      (match eo with None -> () | Some e -> pps " " ; print_exp_aux 0 fmt e) ;
      pps ";"
  ...
  | While (e, b) ->
      pps "while (" ; print_exp_aux 0 fmt e ; pps ") " ; print_block_aux fmt b
...
let print_fdecl_aux fmt {elt= {frtyp; fname; args; body}} =
  ...
  print_list_aux fmt
    (fun () -> pps "," ; ppsp ())
    (fun fmt (t, id) -> print_ty_aux fmt t ; pps " " ; print_id_aux fmt id)
    args ;
  pp_close_box fmt () ;
  pps ") " ;
...
let print_field fmt f =
  let pps = pp_print_string fmt in
  pp_open_hbox fmt () ; print_ty_aux fmt f.ftyp ; pps " " ; pps f.fieldName ; pp_close_box fmt ()

let print_decl_aux fmt g =
  match g with Gvdecl d -> print_gdecl_aux fmt d.elt | Gfdecl f -> print_fdecl_aux fmt f
...
let rec ml_string_of_ty (t : ty) : string =
  match t with TBool -> "TBool" | TInt -> "TInt" | TRef r -> sp "TRef (%s)" (ml_string_of_reft r)

and ml_string_of_ret_ty r = match r with RetVoid -> "TVoid" | RetVal t -> ml_string_of_ty t
...
and ml_string_of_reft (r : rty) : string =
  ...
  | RString ->
      "RString"
  | RArray t ->
      sp "(RArray (%s))" (ml_string_of_ty t)
  | RFun (ts, t) ->
      sp "RFun (%s, %s)" (ml_string_of_list ml_string_of_ty ts) (ml_string_of_ret_ty t)
...
let rec ml_string_of_exp_aux (e : exp) : string =
  ...
  | CStr s ->
      sp "CStr %S" s
  | CArr (t, cs) ->
      sp "CArr (%s,%s)" (ml_string_of_ty t) (ml_string_of_list ml_string_of_exp cs)
  | Id id ->
      sp "Id %s" (ml_string_of_id id)
  | Index (e, i) ->
      sp "Index (%s, %s)" (ml_string_of_exp e) (ml_string_of_exp i)
  | Call (e, exps) ->
      sp "Call (%s, %s)" (ml_string_of_exp e) (ml_string_of_list ml_string_of_exp exps)
  | NewArr (t, e1) ->
      sp "NewArr (%s,%s)" (ml_string_of_ty t) (ml_string_of_exp e1)
  | Bop (b, e1, e2) ->
      sp "Bop (%s,%s,%s)" (ml_string_of_binop b) (ml_string_of_exp e1) (ml_string_of_exp e2)
  | Uop (u, e) ->
      sp "Uop (%s, %s)" (ml_string_of_unop u) (ml_string_of_exp e)

and ml_string_of_exp (e : exp node) : string = ml_string_of_node ml_string_of_exp_aux e
...
let ml_string_of_vdecl (d : vdecl node) : string = ml_string_of_node ml_string_of_vdecl_aux d

let rec ml_string_of_stmt_aux (s : stmt) : string =
  match s with
  | Assn (p, e) ->
      sp "Assn (%s,%s)" (ml_string_of_exp p) (ml_string_of_exp e)
  | Decl d ->
      sp "Decl (%s)" (ml_string_of_vdecl_aux d)
  | Ret e ->
      sp "Ret (%s)" (ml_string_of_option ml_string_of_exp e)
  | SCall (exp, exps) ->
      sp "SCall (%s, %s)" (ml_string_of_exp exp) (ml_string_of_list ml_string_of_exp exps)
  | If (e, b1, b2) ->
      sp "If (%s,%s,%s)" (ml_string_of_exp e) (ml_string_of_block b1) (ml_string_of_block b2)
  | For (d, e, s, b) ->
      sp "For (%s,%s,%s,%s)"
...
and ml_string_of_stmt (s : stmt node) : string = ml_string_of_node ml_string_of_stmt_aux s

and ml_string_of_block (b : block) : string = ml_string_of_list ml_string_of_stmt b

let ml_string_of_args : (ty * id) list -> string =
  ml_string_of_list (fun (t, i) -> sp "(%s,%s)" (ml_string_of_ty t) (ml_string_of_id i))

let rec ml_string_of_fdecl_aux (f : fdecl) : string =
  sp "{ frtyp = %s; name = %s; args = %s; body = %s }" (ml_string_of_ret_ty f.frtyp)
    (ml_string_of_id f.fname) (ml_string_of_args f.args) (ml_string_of_block f.body)

and ml_string_of_fdecl (f : fdecl node) : string = ml_string_of_node ml_string_of_fdecl_aux f

let ml_string_of_gdecl_aux (gd : gdecl) : string =
  sp "{ name = %s; init = %s }" (ml_string_of_id gd.name) (ml_string_of_exp gd.init)

let ml_string_of_gdecl (d : gdecl node) : string = ml_string_of_node ml_string_of_gdecl_aux d

let ml_string_of_decl : decl -> string = function
  | Gvdecl d ->
      sp "Gvdecl (%s)" (ml_string_of_gdecl d)
  | Gfdecl f ->
      sp "Gfdecl (%s)" (ml_string_of_fdecl f)
...
let eq_option (f : 'a -> 'a -> bool) (o1 : 'a option) (o2 : 'a option) : bool =
  match (o1, o2) with None, None -> true | Some a1, Some a2 -> f a1 a2 | _ -> false
...
let rec eq_list (f : 'a -> 'a -> bool) (l1 : 'a list) (l2 : 'a list) : bool =
  ...
  | [], [] ->
      true
  | h1 :: t1, h2 :: t2 ->
      f h1 h2 && eq_list f t1 t2
  | _ ->
      false

let eq_node (f : 'a -> 'a -> bool) (l1 : 'a node) (l2 : 'a node) : bool = f l1.elt l2.elt
...
and eq_rtyp (r1 : rty) (r2 : rty) : bool =
  match (r1, r2) with RString, RString -> true | RArray t1, RArray t2 -> eq_ty t1 t2 | _ -> false
...
let rec eq_exp_aux (e1 : exp) (e2 : exp) : bool =
  ...
  | CStr s1, CStr s2 ->
      s1 = s2
  | CArr (t1, cs1), CArr (t2, cs2) ->
      eq_ty t1 t2 && eq_list eq_exp cs1 cs2
  | Id i1, Id i2 ->
      eq_id i1 i2
  | Index (e1, i1), Index (e2, i2) ->
      eq_exp e1 e2 && eq_exp i1 i2
  | Call (i1, es1), Call (i2, es2) ->
      eq_exp i1 i2 && eq_list eq_exp es1 es2
...
let eq_vdecl_aux ((i1, e1) : vdecl) ((i2, e2) : vdecl) : bool = eq_id i1 i2 && eq_exp e1 e2

let eq_vdecl (d1 : vdecl node) (d2 : vdecl node) : bool = eq_node eq_vdecl_aux d1 d2

let rec eq_stmt_aux (s1 : stmt) (s2 : stmt) : bool =
  match (s1, s2) with
  | Assn (p1, e1), Assn (p2, e2) ->
      eq_exp p1 p2 && eq_exp e1 e2
  | Decl d1, Decl d2 ->
      eq_vdecl_aux d1 d2
  | Ret eo1, Ret eo2 ->
      eq_option eq_exp eo1 eo2
  | SCall (x1, es1), SCall (x2, es2) ->
      eq_exp x1 x2 && eq_list eq_exp es1 es2
  | If (e1, b11, b12), If (e2, b21, b22) ->
      eq_exp e1 e2 && eq_block b11 b21 && eq_block b12 b22
  | For (ds1, eo1, s1, b1), For (ds2, eo2, s2, b2) ->
      eq_list eq_vdecl_aux ds1 ds2 && eq_option eq_exp eo1 eo2 && eq_option eq_stmt s1 s2
      && eq_block b1 b2
  | While (e1, b1), While (e2, b2) ->
      eq_exp e1 e2 && eq_block b1 b2
  | _ ->
      false
...
let rec eq_fdecl_aux (f1 : fdecl) (f2 : fdecl) : bool =
  eq_ret_ty f1.frtyp f2.frtyp && eq_id f1.fname f2.fname && eq_args f1.args f2.args
  && eq_block f1.body f2.body

and eq_fdecl (f1 : fdecl node) (f2 : fdecl node) : bool = eq_node eq_fdecl_aux f1 f2

let eq_gdecl_aux (g1 : gdecl) (g2 : gdecl) : bool = eq_id g1.name g2.name && eq_exp g1.init g2.init

let eq_gdecl (d1 : gdecl node) (d2 : gdecl node) : bool = eq_node eq_gdecl_aux d1 d2
