...
let compile_operand ctxt dest : Ll.operand -> ins = function
  | Null ->
      Asm.(Movq, [~$0; dest])
  | Const i ->
      Asm.(Movq, [Imm (Lit i); dest])
  | Gid id ->
      Asm.(Leaq, [Ind3 (Lbl (Platform.mangle id), Rip); dest])
  | Id id ->
      Asm.(Movq, [lookup ctxt.layout id; dest])
...
let compile_call ctxt (fop : Ll.operand) args =
  ...
  let call_code, op =
    match fop with
    | Gid g ->
        ([], Imm (Lbl (Platform.mangle g)))
    | Id _ ->
        ([op_to_rax fop], Reg Rax)
    | _ ->
        failwith "call function operand was not a local or global id"
  in
  let arg_code =
      ...
      List.fold_left
        (fun (i, r, s) (_, op) ->
          let r, s =
            ...
            | Some reg ->
                (r @ (op_to_rax op :: Asm.[(Movq, [~%Rax; reg])]), s)
            | None ->
                (r, (op_to_rax op :: Asm.[(Pushq, [~%Rax])]) @ s)
          in
          (i + 1, r, s) )
        (0, [], []) args
    in
    register_arg_code @ stack_arg_code
  in
  arg_code @ call_code
  @ (X86.Callq, [op])
    ::
    ( if List.length args > 6 then Asm.[(Addq, [imm_of_int (8 * (List.length args - 6)); ~%Rsp])]
    else [] )
...
let rec size_ty tdecls (t : Ll.ty) : int =
  ...
  | Void | I8 | Fun _ ->
      0
  | I1 | I64 | Ptr _ ->
      8 (* Target 64-bit only subset of X86 *)
  | Struct ts ->
      List.fold_left (fun acc t -> acc + size_ty tdecls t) 0 ts
  | Array (n, t) ->
      n * size_ty tdecls t
  | Namedt id ->
      size_ty tdecls (List.assoc id tdecls)
...
let index_into tdecls (ts : Ll.ty list) (n : int) : int * Ll.ty =
  let rec loop ts n acc =
    ...
    | u :: _, 0 ->
        (acc, u)
    | u :: us, n ->
        loop us (n - 1) (acc + size_ty tdecls u)
    | _ ->
        failwith "index_into encountered bogus index"
  in
  loop ts n 0
...
let compile_gep ctxt (op : Ll.ty * Ll.operand) (path : Ll.operand list) : ins list =
  ...
  let rec loop (ty : Ll.ty) (path : Ll.operand list) code =
    ...
    | Array (_, u), offset_op :: rest ->
        loop u rest
        @@ Asm.[(Addq, [~%Rcx; ~%Rax]); (Imulq, [imm_of_int @@ size_ty ctxt.tdecls u; ~%Rax])]
        @ (op_to_rax offset_op :: Asm.(Movq, [~%Rax; ~%Rcx]) :: code)
    | Namedt t, p ->
        loop (List.assoc t ctxt.tdecls) p code
    | _ ->
        failwith "compile_gep encountered unsupported getelementptr data"
  in
  match op with
  | Ptr t, op ->
      loop (Array (0, t)) path [op_to_rax op]
  | _ ->
      failwith "compile_gep got incorrect parameters"
...
let compile_insn ctxt ((uid, i) : Ll.uid * Ll.insn) : X86.ins list =
  ...
  let op_to_rcx = op_to (Reg Rcx) in
  (* Move the value of op into rax *)
  let dst = lookup ctxt.layout uid in
  match i with
  | Binop (bop, t, op1, op2) -> (
      let bin op =
        op_to_rax op1 :: op_to_rcx op2 :: Asm.[(op, [~%Rcx; ~%Rax]); (Movq, [~%Rax; dst])]
      in
      match bop with
      | Ll.Add ->
          bin Addq
...
let compile_terminator ctxt t =
  let epilogue = Asm.[(Movq, [~%Rbp; ~%Rsp]); (Popq, [~%Rbp]); (Retq, [])] in
  match t with
  | Ll.Ret (_, None) ->
      epilogue
  | Ll.Ret (_, Some o) ->
      compile_operand ctxt (Reg Rax) o :: epilogue
  | Ll.Br l ->
      Asm.[(Jmp, [~$$l])]
  | Ll.Cbr (o, l1, l2) ->
      compile_operand ctxt (Reg Rax) o
      :: Asm.[(Cmpq, [~$0; ~%Rax]); (J X86.Neq, [~$$l1]); (Jmp, [~$$l2])]
...
let compile_fdecl tdecls name ({f_ty; f_param; f_cfg} : Ll.fdecl) =
  ...
  let init_arg_code =
    List.mapi
      (fun i uid -> Asm.[(Movq, [arg_loc i; ~%Rax]); (Movq, [~%Rax; lookup layout uid])])
      f_param
    |> List.flatten
  in
  ...
  let prologue =
    Asm.[(Pushq, [~%Rbp]); (Movq, [~%Rsp; ~%Rbp]); (Subq, [~$tmpsize; ~%Rsp])] @ init_arg_code
  in
...
let rec compile_ginit : Ll.ginit -> X86.data list = function
  | GNull ->
      [Quad (Lit 0L)]
  | GGid gid ->
      [Quad (Lbl (Platform.mangle gid))]
  | GInt c ->
      [Quad (Lit c)]
  | GString s ->
      [Asciz s]
  | GArray gs | GStruct gs ->
      List.map compile_gdecl gs |> List.flatten
