...
let print_x86 file asm_str = print_banner file ; print_endline asm_str
...
let write_file (file : string) (out : string) =
  ...
  fprintf channel "%s" out ; close_out channel
...
let parse_ll_file filename =
  let program = read_file filename |> Lexing.from_string |> Llparser.prog Lllexer.token in
  program
...
let parse_oat_file filename =
  ...
  with Parser.Error ->
    failwith
    @@ Printf.sprintf "Parse error at: %s" (Range.string_of_range (Range.lex_range lexbuf))
...
let process_file path =
  ...
  match ext with
  | "oat" ->
      process_oat_file path basename
  | "ll" ->
      process_ll_file path basename
  | "o" ->
      add_link_file path
  | "c" ->
      add_link_file path
  | _ ->
      failwith @@ Printf.sprintf "found unsupported file type: %s" path

(* process each file separately and then link all of them together *)
let process_files files =
  if List.length files > 0 then (
    List.iter process_file files ;
    if !assemble && !link then Platform.link (List.rev !link_files) !executable_filename ;
