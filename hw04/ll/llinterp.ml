...
(* The memory state *)
type config = {globals: (gid * mval) list; heap: (mid * mval) list; stack: (fid * mval) list}

(* Create memory value for global declaration *)
let mval_of_gdecl (gd : gdecl) : mval =
  let rec mtree_of_gdecl : gdecl -> mtree = function
    | ty, GNull ->
        MWord (VPtr (ty, NullId, [0]))
    | ty, GGid g ->
        MWord (VPtr (ty, GlobId g, [0]))
    | _, GInt i ->
        MWord (VInt i)
    | _, GString s ->
        MStr s
    | _, GArray gs | _, GStruct gs ->
        MNode (List.map mtree_of_gdecl gs)
  in
  [mtree_of_gdecl gd]
...
let mval_of_ty (nt : tid -> ty) (t : ty) : mval =
  let rec mtree_of_ty : ty -> mtree = function
    | I1 | I8 | I64 | Ptr _ ->
        MWord VUndef
    | Array (n, I8) ->
        MStr (String.make n '\x00')
    | Array (n, t) ->
        MNode Array.(make n (MWord VUndef) |> to_list)
    | Struct ts ->
        MNode (List.map mtree_of_ty ts)
    | Fun _ | Void ->
        failwith "mval_of_ty: mval for bad type"
    | Namedt id ->
        mtree_of_ty (nt id)
  in
  [mtree_of_ty t]
...
let string_of_sval (sv : sval) : string =
  match sv with VUndef -> "undef" | VInt x -> Int64.to_string x | VPtr p -> string_of_ptr p

let rec string_of_mval (mv : mval) : string = "[" ^ mapcat " " string_of_mtree mv ^ "]"
...
let interp_bop (b : bop) (v1 : sval) (v2 : sval) : sval =
  let i, j = match (v1, v2) with VInt i, VInt j -> (i, j) | _ -> invalid_arg "interp_bop" in
  let open Int64 in
  let f =
    match b with
    | Add ->
    ...
    | Xor ->
        logxor
    | Shl ->
        fun i j -> shift_left i @@ to_int j
    | Lshr ->
        fun i j -> shift_right_logical i @@ to_int j
    | Ashr ->
        fun i j -> shift_right i @@ to_int j
  in
  VInt (f i j)
...
let interp_cnd (c : cnd) (v1 : sval) (v2 : sval) =
  ...
  in
  match (v1, v2, c) with
  | VPtr (_, b1, i1), VPtr (_, b2, i2), Eq | VPtr (_, b1, i1), VPtr (_, b2, i2), Ne ->
      VInt (if f (b1, i1) (b2, i2) then 1L else 0L)
  | VInt i, VInt j, _ ->
      VInt (if f i j then 1L else 0L)
  | _ ->
      invalid_arg "interp_cnd"
...
let rec interp_operand (nt : tid -> ty) (locs : locals) (ty : ty) (o : operand) : sval =
  ...
  | _, Id u ->
      locs u
  | Namedt id, o ->
      interp_operand nt locs (nt id) o
  | _ ->
      failwith @@ "interp_operand: malformed operand " ^ soo o ^ ":" ^ sot ty

(* Some utility functions *)
let update f k v k' = if k = k' then v else f k'

let rec is_prefix (l : 'a list) (m : 'a list) : bool =
  match (l, m) with [], _ -> true | _, [] -> false | a :: l, b :: m -> a = b && is_prefix l m
...
let replace_nth (l : 'a list) (n : int) (a : 'a) : 'a list =
  let rec loop acc n = function
    | [] ->
        raise Not_found
    | a' :: l' ->
        if n = 0 then List.rev_append acc (a :: l') else loop (a' :: acc) (pred n) l'
  in
  loop [] n l
...
let rec load_idxs (m : mval) (idxs : idx list) : mtree =
  ...
  | i :: idxs' -> (
      ...
      if len <= i || i < 0 then raise OOBIndexDeref
        ...
        | _, MWord v ->
            failwith "load_idxs: attempted to index into word"
        | _, MStr _ ->
            failwith "load_idxs: attempted to index into string"
        | _, MNode m' ->
            load_idxs m' idxs' )
  ...
  | i :: idxs' -> (
      ...
      if len <= i || i < 0 then raise OOBIndexDeref
        ...
        | [], _ ->
            replace_nth m i mt
        | _, MWord v ->
            failwith "store_idxs: attempted to index into word"
        | _, MStr _ ->
            failwith "store_idxs: attempted to index into string"
        | _, MNode m' ->
            replace_nth m i @@ MNode (store_idxs m' idxs' mt) )
...
let load_bid (c : config) (bid : bid) : mval =
  ...
  | GlobId gid -> (
    try List.assoc gid c.globals with Not_found -> failwith "Load: bogus gid" )
  | StckId fid -> (
    try List.assoc fid c.stack with Not_found -> raise UseAfterFree )

let load_ptr (c : config) ((_, bid, idxs) : ptr) : mtree = load_idxs (load_bid c bid) idxs
...
let effective_tag (nt : tid -> ty) ((tag, _, idxs) : ptr) : ty =
  let rec loop tag idxs =
    ...
    | t, [] ->
        t
    | Struct ts, i :: idxs' ->
        if List.length ts <= i then failwith "effective_tag: index oob of struct"
        else loop (List.nth ts i) idxs'
    | Array (n, t), i :: idxs' ->
        loop t idxs' (* Don't check if OOB! *)
    | Namedt id, _ ->
        loop (nt id) idxs
    | _, _ :: _ ->
        failwith "effective_tag: index into non-aggregate"
  in
  loop tag
  @@
  try List.tl idxs
...
let rec gep_idxs (p : idx list) (idxs : idx list) : idx list =
  match (p, idxs) with
  | [], _ | _, [] ->
      failwith "gep_idxs: invalid indices"
  | [i], j :: idxs' ->
      (i + j) :: idxs'
  | i :: p', _ ->
      i :: gep_idxs p' idxs
...
let gep_ptr (nt : tid -> ty) (ot : ty) (p : ptr) (idxs' : idx list) : sval =
  ...
  else match p with t, NullId, idxs -> VUndef | t, bid, idxs -> VPtr (t, bid, gep_idxs idxs idxs')
...
let interp_prog {tdecls; gdecls; fdecls; edecls= _} (args : string list) : sval =
  ...
  let runtime_call (t : ty) (fn : gid) (args : sval list) (c : config) : config * sval =
    let load_strptr p =
      match load_ptr c p with MStr s -> s | _ -> failwith "runtime_call: non string-ptr arg"
    in
    match (t, fn, args) with
    | Void, "ll_puts", [VPtr p] ->
        let s = load_strptr p in
        print_endline s ; (c, VUndef)
    ...
    | I64, "ll_ltoa", [VInt i; VPtr dst] ->
        ...
        ({c with heap}, VPtr (t, HeapId mid, [0]))
    | _ ->
        failwith @@ "runtime_call: invalid call to " ^ fn
  in
  ...
  let rec interp_cfg ((k, blocks) : cfg) (locs : locals) (c : config) : config * sval =
    ...
    | (u, Load (Ptr t, o)) :: insns, _ ->
        let mt =
          ...
          | VPtr p ->
              if effective_tag nt p <> t then raise IncompatTagDeref else load_ptr c p
          | VUndef ->
              raise UndefPtrDeref
          | VInt _ ->
              failwith "non-ptr arg for load"
        in
        let v =
          match mt with
          | MWord VUndef ->
          ...
          | MWord v ->
              v
          | _ ->
              failwith "load: returned aggregate"
        in
    ...
    | (_, Store (t, os, od)) :: insns, _ ->
        ...
        let c' =
          match vd with
          | VPtr p ->
              if effective_tag nt p <> t then raise IncompatTagDeref else store_ptr c p (MWord vs)
          | VUndef ->
              raise UndefPtrDeref
          | VInt _ ->
              failwith "non-vptr arg for load"
        in
        interp_cfg ({k with insns}, blocks) locs c'
    ...
    | (u, Gep (Ptr t, o, os)) :: insns, _ ->
        let idx_of_sval : sval -> idx = function
          | VInt i ->
              Int64.to_int i
          | _ ->
              failwith "idx_of_sval: non-integer value"
        in
    ...
    | [], (_, Cbr (o, l1, l2)) ->
        ...
        interp_cfg (k', blocks) locs c
    | (u, i) :: _, _ ->
        failwith @@ "interp_cfg: invalid instruction \"" ^ string_of_insn i ^ "\" at %" ^ u
