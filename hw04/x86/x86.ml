...
type asm = Text of ins list (* code *) | Data of data list
...
let string_of_byte_reg : reg -> string = function
  | Rip ->
      failwith "%rip used as byte register"
  | Rax ->
      "%al"
  | Rbx ->
      "%bl"
...
let string_of_ins ((op, args) : ins) : string =
  match op with
  | Shlq | Sarq | Shrq ->
      string_of_shift op args
  | _ ->
      let f =
        match op with
        | J _ | Jmp | Callq ->
            string_of_jmp_operand
        | Set _ ->
            string_of_byte_operand
        | _ ->
            string_of_operand
      in
...
let string_of_data : data -> string = function
  | Asciz s ->
      "\t.asciz\t" ^ "\"" ^ String.escaped s ^ "\""
  | Quad i ->
      "\t.quad\t" ^ string_of_imm i
...
let string_of_prog (p : prog) : string = String.concat "\n" @@ List.map string_of_elem p
