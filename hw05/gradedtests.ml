...
let io_test path args =
  ...
  let _ =
    Platform.sh
      (Printf.sprintf "rm -f %s %s %s" dot_s_file exec_file tmp_file)
...
let oat_tc_err_file_test path err =
  ...
  with Typechecker.TypeError s ->
    if s = err then () else failwith @@ Printf.sprintf "Expected type error: %s but got %s" err s

let executed tests =
  List.map (fun (fn, ans) -> (fn, assert_eqf (fun () -> exec_e2e_file fn "") ans)) tests
...
let executed_tc_ok_file tests =
  List.map (fun path -> ("typechecking: " ^ path, fun () -> oat_tc_ok_file_test path)) tests

let executed_tc_err_file tests =
  List.map
    (fun (path, err) -> ("typechecking: " ^ path, fun () -> oat_tc_err_file_test path err))
    tests
...
let typecheck_file_error tests = List.map (fun p -> (p, typecheck_error (typecheck p))) tests

let typecheck_file_correct tests = List.map (fun p -> (p, typecheck_correct (typecheck p))) tests
...
let typecheck_equality_tests = ["hw5programs/tc_eq1.oat"; "hw5programs/tc_eq2.oat"]
...
let typecheck_error_null_not_null_tests = hw4_type_error_tests
...
let typecheck_tests : suite =
  ...
  ; GradedTest
      ("tc statement correct tests", 5, typecheck_file_correct typecheck_correct_statement_tests)
  ; GradedTest ("tc other correct tests", 5, typecheck_file_correct typecheck_correct_other_tests)
  ; GradedTest
      ("tc null/not null error tests", 5, typecheck_file_error typecheck_error_null_not_null_tests)
  ; GradedTest
      ("tc expression error tests", 5, typecheck_file_error typecheck_error_expression_tests)
  ; GradedTest
      ( "tc struct/global error tests"
      , 5
      , typecheck_file_error (typecheck_error_struct_tests @ typecheck_error_global_tests) )
...
let manual_tests : suite =
  [ GradedTest ("Posted Piazza Test Case", 10, [("manually", assert_eq true false)])
  ; GradedTest ("Other Student Piazza Tests", 5, [("manually", assert_eq true false)]) ]

let hw4_tests =
  hw4_easiest_tests @ hw4_globals_tests @ hw4_path_tests @ hw4_easy_tests @ hw4_medium_tests
  @ hw4_hard_tests @ hw4_old_student_tests

let functionality_tests : suite =
  [GradedTest ("functionality tests from HW04", 10, executed_oat_file hw4_tests)]

let graded_tests : suite = typecheck_tests @ hw5_tests @ functionality_tests @ manual_tests
