...
let assert_eq v1 v2 : assertion = fun () -> if v1 <> v2 then failwith "not equal" else ()

let assert_eqf f v2 : assertion = fun () -> if f () <> v2 then failwith "not equal" else ()
...
let timeout_suite (time : int) (s : suite) : suite = List.map (timeout_test time) s
...
let result_test_to_string (name_pts : string) (r : result test) : string =
  let string_of_case (name, res) =
    match res with Pass -> "passed - " ^ name | Fail msg -> "FAILED - " ^ name ^ ": " ^ msg
  in
  match r with
  | GradedTest (_, _, cases) | Test (_, cases) ->
      name_pts ^ List.fold_left (fun rest case -> rest ^ "\n" ^ string_of_case case) "" cases

(* returns (name_pts, passed, failed, total, points_earned, max_given, max_hidden) *)
let get_results (t : result test) =
  let num_passed cases =
    List.fold_left (fun cnt (_, r) -> match r with Pass -> cnt + 1 | _ -> cnt) 0 cases
  in
  let num_failed cases =
    List.fold_left (fun cnt (_, r) -> match r with Fail _ -> cnt + 1 | _ -> cnt) 0 cases
  in
  match t with
  | GradedTest (name, pts, cases) ->
      ...
      if total > 0 then
        let points_earned =
          floor (float_of_int passed /. float_of_int total) *. float_of_int pts
        in
...
let outcome_to_string (o : outcome) : string =
  ...
  str ^ sep
  ^ Printf.sprintf "Passed: %d/%d\nFailed: %d/%d\nScore: %1.f/%d (given)\n       ?/%d (hidden)" p
      tot f tot pts maxg maxh
