...
(* TASK: complete the flow function for alias analysis.
   ...
   - A pointer returned by a load, call, bitcast, or GEP may be aliased
     (note: since this pointer is stored in uid, that too may be aliased)
...
module Fact = struct
  ...
  (* TASK: complete the "combine" operation for alias analysis.
    ...
    *)
  let combine (ds : fact list) : fact = failwith "Alias.Fact.combine not implemented"
end
...
let analyze (g : Cfg.t) : Graph.t =
  ...
  let alias_in =
    List.fold_right
      (fun (u, t) -> match t with Ptr _ -> UidM.add u SymPtr.MayAlias | _ -> fun m -> m)
