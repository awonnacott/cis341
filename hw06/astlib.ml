...
and print_exp_aux level fmt e =
  ...
  | CArr (ty, vs) ->
      ...
      pps "}"
  | Length l ->
      pps "length( " ; print_exp_aux this_level fmt l ; pps " )"
  | Id id ->
      print_id_aux fmt id
  | Index (e, i) ->
...
let print_decl_aux fmt g =
  match g with
  | Gvdecl d ->
      print_gdecl_aux fmt d.elt
  | Gfdecl f ->
      print_fdecl_aux fmt f
  | Gtdecl t ->
      print_tdecl_aux fmt t.elt
...
let rec ml_string_of_ty (t : ty) : string =
  ...
  | TInt ->
      "TInt"
  | TRef r ->
      sp "TRef (%s)" (ml_string_of_reft r)
  | TNullRef r ->
      sp "TNullRef (%s)" (ml_string_of_reft r)
...
let rec ml_string_of_exp_aux (e : exp) : string =
  match e with
  | CNull r ->
      sp "CNull %s" (ml_string_of_reft r)
  | CBool b ->
      sp "CBool %b" b
  | CInt i ->
      sp "CInt %LiL" i
  ...
  | NewArr (t, e1, u, e2) ->
      sp "NewArr (%s,%s,%s,%s)" (ml_string_of_ty t) (ml_string_of_exp e1) (ml_string_of_id u)
        (ml_string_of_exp e2)
  | Proj (exp, id) ->
  ...
  | Length e ->
      sp "Length (%s)" (ml_string_of_exp e)
...
let rec ml_string_of_fdecl_aux (f : fdecl) : string =
  sp "{ rtyp = %s; name = %s; args = %s; body = %s }" (ml_string_of_ret_ty f.frtyp)
...
let ml_string_of_field {fieldName; ftyp} : string =
  sp "{ fname = %s; typ = %s }" (ml_string_of_id fieldName) (ml_string_of_ty ftyp)

let ml_string_of_tdecl_aux (id, fs) : string =
  sp "(id = %s, fs = (%s))" (ml_string_of_id id) (ml_string_of_list ml_string_of_field fs)

let ml_string_of_tdecl (t : tdecl node) : string = ml_string_of_node ml_string_of_tdecl_aux t
...
let ml_string_of_decl : decl -> string = function
  ...
  | Gtdecl t ->
      sp "Gtdecl (%s)" (ml_string_of_tdecl t)
