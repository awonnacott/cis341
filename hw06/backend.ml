  ...
  let of_block (f : Ll.uid -> loc) (g : Ll.gid -> X86.lbl) (live : Ll.uid -> UidSet.t)
    ...
    List.map
      (fun (u, i) ->
        ...
        (map_insn f g (u, i), map_lset f @@ live u) )
      b.insns
    @
    let x, t = b.term in
...
let compile_operand : Alloc.operand -> X86.operand = function
  ...
  | Gid l ->
      Asm.(~$$l)
  | Loc LVoid ->
      failwith "compiling uid without location"
  | Loc (LStk i) ->
...
let emit_mov (src : X86.operand) (dst : X86.operand) : x86stream =
  ...
  | Reg r, Reg r' when r = r' ->
      []
  | Reg _, _ ->
      lift Asm.[(Movq, [src; dst])]
  | _, Reg _ ->
      lift Asm.[(Movq, [src; dst])]
  | _, _ ->
      lift Asm.[(Pushq, [src]); (Popq, [dst])]
...
let compile_pmov live (ol : (Alloc.loc * Ll.ty * Alloc.operand) list) : x86stream =
  ...
  let rec loop outstream ol =
    ...
    | ol', ready ->
        loop
          (List.fold_left
             (fun os (x, _, o) -> os >@ emit_mov (compile_operand o) (compile_operand (Loc x)))
             outstream ready)
          ol'
  in
  loop [] ol'
  ...
  let oreg, ostk, _ =
    List.fold_left
      (fun (oreg, ostk, i) (t, o) ->
        ...
        | Some r ->
            ((Alloc.LReg r, t, o) :: oreg, ostk, i + 1)
        | None ->
            (oreg, o :: ostk, i + 1) )
      ([], [], 0) os
  in
    ...
    | Array (_, u), offset_op :: rest ->
        ...
        @@ ( code
           >@ ( [I Asm.(Movq, [~%Rax; ~%Rcx])]
              ...
              >@ [I Asm.(Addq, [~%Rcx; ~%Rax])] ) )
    | Namedt t, p ->
        loop (List.assoc t tdecls) p code
    | _ ->
        failwith "compile_gep encountered unsupported getelementptr data"
  in
  match t with
  | Ptr t ->
      loop (Array (0, t)) path (emit_mov (compile_operand o) (Reg Rax))
  | _ ->
...
let compile_fbody tdecls (af : Alloc.fbody) : x86stream =
  let rec loop (af : Alloc.fbody) (outstream : x86stream) : x86stream =
    ...
    | [] ->
        outstream
    | (ILbl (LLbl l), _) :: rest ->
        loop rest @@ (outstream >:: L (l, false))
    | (PMov ol, live) :: rest ->
        loop rest @@ (outstream >@ compile_pmov live ol)
    | (Icmp (LVoid, _, _, _, _), _) :: rest ->
        loop rest outstream
    | (Binop (LVoid, _, _, _, _), _) :: rest ->
        loop rest outstream
    | (Alloca (LVoid, _), _) :: rest ->
        loop rest outstream
    | (Bitcast (LVoid, _, _, _), _) :: rest ->
        loop rest outstream
    | (Load (LVoid, _, _), _) :: rest ->
        loop rest outstream
    | (Gep (LVoid, _, _, _), _) :: rest ->
        loop rest outstream
    | (Icmp (x, c, _, Loc (LReg o), o'), _) :: rest ->
        loop rest
        @@ ( outstream
           >@ lift Asm.[(Cmpq, [co o'; ~%o]); (cc c, [co (Loc x)]); (Andq, [~$1; co (Loc x)])] )
    | (Icmp (x, c, _, o, o'), _) :: rest ->
        loop rest
        @@ ( outstream
           >@ emit_mov (co o) (Reg Rax)
           >@ lift Asm.[(Cmpq, [co o'; ~%Rax]); (cc c, [co (Loc x)]); (Andq, [~$1; co (Loc x)])] )
    ...
    | (Alloca (x, at), _) :: rest ->
        loop rest
        @@ ( outstream
           >@ lift Asm.[(Subq, [~$(size_ty tdecls at); ~%Rsp]); (Movq, [~%Rsp; co (Loc x)])] )
    ...
    | _ ->
        failwith "codegen failed to find instruction"
  in
  loop af []
...
(* TASK: Implement a (correct) register allocation strategy that
   ...
   To see whether your register assignment is better than the greedy linear scan one,
   we check:
...
let rec compile_ginit : Ll.ginit -> X86.data list = function
  ...
  | GArray gs | GStruct gs ->
      List.(flatten @@ map compile_gdecl gs)
