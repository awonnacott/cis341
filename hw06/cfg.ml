...
type cfg =
  {blocks: Ll.block LblM.t; preds: LblS.t LblM.t; ret_ty: Ll.ty; args: (Ll.uid * Ll.ty) list}
...
let of_ast (fdecl : Ll.fdecl) : cfg =
  ...
  let blocks = List.fold_left (fun g (l, block) -> LblM.add l block g) LblM.empty ast in
...
let add_block (l : lbl) (block : block) (g : cfg) : cfg = {g with blocks= LblM.add l block g.blocks}
...
module AsGraph (D : AS_GRAPH_PARAMS) : sig
  ...
  let of_cfg init flow_in cfg =
    let dfa = cfg.blocks |> LblM.mapi (fun l _ -> init l) |> LblM.add bound_lbl flow_in in
    {cfg; dfa}
  ...
  let dfa_preds =
    if D.forwards then fun g ->
      preds g.cfg |> extend (ns entry_lbl) (ns bound_lbl) |> extend (ns bound_lbl) NodeS.empty
    else fun g ->
      succs g.cfg |> extend (exits g.cfg) (ns bound_lbl) |> extend (ns bound_lbl) NodeS.empty

  let dfa_succs =
    if D.forwards then fun g -> extend (ns bound_lbl) (ns entry_lbl) (succs g.cfg)
  ...
  let flow g (l : lbl) =
    if Lbl.compare l bound_lbl == 0 then fun _ -> LblM.find l g.dfa else flow_block g l
