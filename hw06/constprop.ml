...
module SymConst = struct
  ...
  let compare s t =
    ...
    | Const i, Const j ->
        Int64.compare i j
    | NonConst, NonConst | UndefConst, UndefConst ->
        0
    | NonConst, _ | _, UndefConst ->
        1
    | UndefConst, _ | _, NonConst ->
        -1
...
module Fact = struct
  ...
  let combine (ds : fact list) : fact = failwith "Constprop.Fact.combine unimplemented"
end
...
let analyze (g : Cfg.t) : Graph.t =
  ...
  let cp_in = List.fold_right (fun (u, _) -> UidM.add u SymConst.NonConst) g.Cfg.args UidM.empty in
