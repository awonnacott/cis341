...
module UidGraph = struct
  ...
  let add_edge graph u v =
    UidM.update_or UidS.empty (UidS.add v) u
      (UidM.update_or UidS.empty (UidS.add u) v graph)

  (** Add a vertex with no neighbors *)
  let add_vertex graph v =
    UidM.update_or UidS.empty (fun x -> x) v graph
  ...
  let degree graph v =
    UidS.cardinal (UidM.find_or UidS.empty graph v)

  (** Remove a vertex (and all of its incident edges) from a graph *)
  let remove_vertex graph v =
    UidM.map (UidS.remove v) (UidM.remove v graph)
  ...
  let min_vertex min graph =
    ...
    | [] -> None
    | ((x,_)::xs) -> Some (List.fold_left (fun v (y, _) -> min v y) x xs)

  (** Print the edges of a graph *)
  let print graph =
    Llutil.mapcat "\n" (fun (u, s) -> Printf.sprintf "%s: %s" u (UidS.to_string s)) (UidM.bindings graph)
end
