  ...
  let gs, einsns, insns, term_opt, blks =
    List.fold_left
      (fun (gs, einsns, insns, term_opt, blks) e ->
        ...
        | I (uid, insn) ->
            (gs, einsns, (uid, insn) :: insns, term_opt, blks)
        | G (gid, gdecl) ->
            ((gid, gdecl) :: gs, einsns, insns, term_opt, blks)
        | E (uid, i) ->
            (gs, (uid, i) :: einsns, insns, term_opt, blks) )
      ([], [], [], None, []) code
  in
  match term_opt with
  | None ->
      failwith "build_cfg: entry block has no terminator"
  | Some term ->
...
and cmp_rty ct : Ast.rty -> Ll.ty = function
  | Ast.RString ->
      I8
  | Ast.RArray u ->
      Struct [I64; Array (0, cmp_ty ct u)]
  | Ast.RStruct r ->
      Namedt r
  | Ast.RFun (ts, t) ->
...
let rec size_oat_struct (l : Ast.field list) =
  match l with [] -> 0L | f :: t -> Int64.(add (size_oat_struct t) (size_oat_ty f.ftyp))
...
let oat_alloc_struct ct (id : Ast.id) : Ll.ty * operand * stream =
  ...
  , lift
      [ ( arr_id
        , Call (arr_ty, Gid "oat_malloc", [(I64, Const (size_oat_struct (TypeCtxt.lookup id ct)))])
        )
...
let rec cmp_exp (tc : TypeCtxt.t) (c : Ctxt.t) (exp : Ast.exp node) : Ll.ty * Ll.operand * stream =
  ...
  | Ast.Uop (uop, e) ->
      ...
      let cmp_uop op = function
        ...
        | Ast.Lognot ->
            Icmp (Eq, I1, op, i1_op_of_bool false)
        | Ast.Bitnot ->
            Binop (Xor, I64, op, i64_op_of_int (-1))
      in
  ...
  | Ast.Length e ->
      ...
      let _ =
        ...
        | Ptr (Struct [_; Array (_, t)]) ->
            t
        | _ ->
            failwith "Length: indexed into non pointer"
      in
...
and cmp_exp_lhs (tc : TypeCtxt.t) (c : Ctxt.t) (e : exp node) : Ll.ty * Ll.operand * stream =
  ...
  | Ast.Index (e, i) ->
      ...
      let ans_ty =
        ...
        | Ptr (Struct [_; Array (_, t)]) ->
            t
        | _ ->
            failwith "Index: indexed into non pointer"
      in
  ...
  | _ ->
      failwith "invalid lhs expression"

and cmp_call (tc : TypeCtxt.t) (c : Ctxt.t) (exp : Ast.exp node) (es : Ast.exp node list)
    : Ll.ty * Ll.operand * stream
  =
  let t, op, s = cmp_exp tc c exp in
  let ts, rt =
    match t with Ptr (Fun (l, r)) -> (l, r) | _ -> failwith "nonfunction passed to cmp_call"
  in
  let args, args_code =
    List.fold_right2
      (fun e t (args, code) ->
...
and cmp_stmt (tc : TypeCtxt.t) (c : Ctxt.t) (rt : Ll.ty) (stmt : Ast.stmt node) : Ctxt.t * stream =
  ...
  | Ast.If (guard, st1, st2) ->
      ...
      , guard_code
        ...
        >:: L lt >@ then_code >:: T (Br lm) >:: L le >@ else_code >:: T (Br lm) >:: L lm )
  ...
  *)
  | Ast.Cast (typ, id, exp, notnull, null) ->
      ...
      , guard_code
        ...
        >@ notnull_code >:: T (Br lm) >:: L ln >@ null_code >:: T (Br lm) >:: L lm )
...
let cmp_global_ctxt (tc : TypeCtxt.t) (c : Ctxt.t) (p : Ast.prog) : Ctxt.t =
  let gexp_ty c : exp -> Ll.ty = function
    ...
    | CStr s ->
        Ptr (str_arr_ty s)
    | CArr (u, cs) ->
        Ptr (Struct [I64; Array (List.length cs, cmp_ty tc u)])
    | x ->
...
let rec cmp_gexp c (tc : TypeCtxt.t) (e : Ast.exp node) : Ll.gdecl * (Ll.gid * Ll.gdecl) list =
  ...
  | CBool b ->
      ((I1, if b then GInt 1L else GInt 0L), [])
  | CInt i ->
      ((I64, GInt i), [])
  | Id id ->
      ((fst @@ Ctxt.lookup id c, GGid id), [])
  | CStr s ->
  ...
  | CStruct (id, cs) ->
      ...
      let elts, gs =
        List.fold_right
          (fun fs (elts, gs) ->
            let gd, gs' =
              cmp_gexp c tc (snd (List.find (fun (xid, xname) -> xid = fs.fieldName) cs))
            in
            (gd :: elts, gs' @ gs) )
          fields ([], [])
      in
  ...
  | _ ->
      failwith "bad global initializer"
...
let cmp_prog (p : Ast.prog) : Ll.prog =
  ...
  let init_ctxt =
    List.fold_left (fun c (i, t) -> Ctxt.add c i (Ll.Ptr t, Gid i)) Ctxt.empty builtins
  in
