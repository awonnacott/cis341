...
let rec forget_size = function
  ...
  | Ptr ty -> Ptr (forget_size ty)
  | Struct fields -> Struct (List.map forget_size fields)
  | Array (_, I8) -> Ptr I8
  | Array (_, ty) -> Array (0, forget_size ty)
  | Fun (args, ret) -> Fun (List.map forget_size args, forget_size ret)
  | Namedt tid -> Namedt tid
  ...
  | Ast.Id id ->
    ...
    begin match t with
      ...
      | Ptr t ->
        ...
        if t = ft then
          ft, Id ans_id, [I(ans_id, Load(Ptr t, op))]
        else
          let res_id = gensym id in
          ft, Id res_id, [ I(res_id, Bitcast(t, Id ans_id, ft))
                         ; I(ans_id, Load(Ptr t, op))]
...
let cmp_fdecl (tc : TypeCtxt.t) (c:Ctxt.t) (f:Ast.fdecl node) : Ll.fdecl * (Ll.gid * Ll.gdecl) list =
  ...
  let return_code =
    let return_val =
      ...
      | RetVal (TRef _) | RetVal (TNullRef _) -> Some Null
    in
