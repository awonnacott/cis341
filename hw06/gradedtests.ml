...
let assert_quality fn ll_ast =
  if not !pass_all then failwith "Your register allocator failed a correctness test"
  else
    let _ = Opt.do_opt := true in
    let ll_ast = Opt.optimize ll_ast in
    ...
    let _ =
      if !Driver.print_regs_flag then
        Printf.printf
          "greedy sz: %4d mem: %4d\t\tbetter sz: %4d mem: %4d \t diff_sz: %4d diff_mem: %4d - %s\n"
          size_greedy mem_greedy size_better mem_better (size_greedy - size_better)
          (mem_greedy - mem_better) fn
    in
    if mem_better < mem_greedy then ()
    else if size_better < size_greedy then ()
...
let quality_oat tests = List.map (fun (fn, _, _) -> (fn, assert_quality_oat fn)) tests

let fdecl_of_path path =
  Platform.verb @@ Printf.sprintf "* processing file: %s\n" path ;
  let ll_ast = parse_ll_file path in
  match ll_ast.Ll.fdecls with [(_, fdecl)] -> fdecl | _ -> failwith "test expected one fdecl"
...
let dfa_liveness_file (tests : (string * 'a Datastructures.LblM.t) list) =
  ...
  List.map
    (fun (path, ans) ->
      ( "liveness: " ^ path
      , fun () -> ll_dfa_file_test path (throw_key_diff Fact.compare) analyze ans ) )
    tests
...
let dfa_alias_file tests =
  ...
  List.map
    (fun (path, ans) ->
      ("alias: " ^ path, fun () -> ll_dfa_file_test path (throw_key_diff Fact.compare) analyze ans)
      )
    tests
...
let opt_dce_file tests =
  ...
  List.map
    (fun (path, ans) ->
      ( Printf.sprintf "dce opt: %s, %s" (Filename.basename path) (Filename.basename ans)
      , fun () -> ll_opt_file_test path opt ans ) )
    tests
...
let opt_constfold_file tests =
  ...
  List.map
    (fun (path, ans) ->
      ( Printf.sprintf "constprop opt: %s, %s" (Filename.basename path) (Filename.basename ans)
      , fun () -> ll_opt_file_test path opt ans ) )
    tests
...
let naive_factor_tests =
  [("llprograms/naive_factor_prime.ll", "1"); ("llprograms/naive_factor_nonprime.ll", "0")]
...
let ll_tests =
  binop_tests @ terminator_tests @ memory_tests @ calling_convention_tests @ bitcast_tests
  @ gep_tests @ arithmetic_tests @ sum_tree_tests @ gcd_euclidian_tests @ binary_search_tests
  @ gep_5_deep_tests @ binary_gcd_tests @ linear_search_tests @ lfsr_tests @ naive_factor_tests
  @ euclid_recursive_test @ matmul_tests @ large_tests
...
let oat_regalloc_quality_tests =
  hw4_easiest_tests @ hw4_path_tests @ hw4_easy_tests @ hw4_medium_tests @ hw4_hard_tests
  @ hw4_old_student_tests @ struct_tests @ fptr_tests @ new_tests @ regalloc_challenge_tests

let oat_correctness_tests = oat_regalloc_quality_tests @ hw4_globals_tests @ greedy_is_good_tests
...
let manual_tests : suite =
  ...
  ; GradedTest ("Performance Comparison", 5, [("manually", assert_eq true false)]) ]
