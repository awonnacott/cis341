...
let rec string_of_ty : ty -> string = function
  ...
  | I64 ->
      "i64"
  | Ptr ty ->
      pp "%s*" (string_of_ty ty)
  | Struct ts ->
      pp "{ %s }" (mapcat ", " string_of_ty ts)
  | Array (n, t) ->
      pp "[%s x %s]" (string_of_int n) (string_of_ty t)
  | Fun (ts, t) ->
      pp "%s (%s)" (string_of_ty t) (mapcat ", " string_of_ty ts)
  | Namedt s ->
      pp "%%%s" s
...
let string_of_insn : insn -> string = function
  ...
  | Alloca t ->
      pp "alloca %s" (sot t)
  | Load (t, o) ->
      pp "load %s, %s %s" (sot (dptr t)) (sot t) (soo o)
  | Store (t, os, od) ->
      pp "store %s %s, %s %s" (sot t) (soo os) (sot (Ptr t)) (soo od)
  | Icmp (c, t, o1, o2) ->
      pp "icmp %s %s %s, %s" (string_of_cnd c) (sot t) (soo o1) (soo o2)
  | Call (t, o, oa) ->
      pp "call %s %s(%s)" (sot t) (soo o) (mapcat ", " soop oa)
  | Bitcast (t1, o, t2) ->
      pp "bitcast %s %s to %s" (sot t1) (soo o) (sot t2)
  | Gep (t, o, oi) ->
...
let string_of_named_insn ((u, i) : uid * insn) : string =
  ...
  | Store _ | Call (Void, _, _) ->
      string_of_insn i
  | _ ->
      pp "%%%s = %s" u (string_of_insn i)

let string_of_terminator : terminator -> string = function
  | Ret (_, None) ->
      "ret void"
  | Ret (t, Some o) ->
      pp "ret %s %s" (sot t) (soo o)
  | Br l ->
      pp "br label %%%s" l
  | Cbr (o, l, m) ->
      pp "br i1 %s, label %%%s, label %%%s" (soo o) l m
...
let rec string_of_ginit : ginit -> string = function
  ...
  | GString s ->
      pp "c\"%s\\00\"" s
  | GArray gis ->
      pp "[ %s ]" (mapcat ", " string_of_gdecl gis)
  | GStruct gis ->
      pp "{ %s }" (mapcat ", " string_of_gdecl gis)

and string_of_gdecl ((t, gi) : gdecl) : string = pp "%s %s" (sot t) (string_of_ginit gi)

let string_of_named_gdecl ((g, gd) : gid * gdecl) : string =
  pp "@%s = global %s" g (string_of_gdecl gd)

let string_of_named_tdecl ((n, t) : tid * ty) : string = pp "%%%s = type %s" n (sot t)
...
let string_of_named_edecl ((g, t) : gid * ty) : string =
  ...
  | _ ->
      pp "@%s = external global %s" g (string_of_ty t)
...
let compare_block (b : block) (c : block) : int =
  let del_dummy (u, i) =
    match i with Store (_, _, _) -> ("", i) | Call (Void, _, _) -> ("", i) | _ -> (u, i)
  in
...
module Parsing = struct
  let gensym, reset =
    let c = ref 0 in
    ((fun (s : string) -> incr c ; Printf.sprintf "_%s__%d" s !c), fun () -> c := 0)
end
