...
let regs = [Rax; Rbx; Rcx; Rdx; Rsi; Rdi; Rbp; Rsp; R08; R09; R10; R11; R12; R13; R14; R15; Rip]
...
let histogram_of_prog (p : prog) : histogram * int =
  let h = mk_empty () in
  let size = ref 0 in
  let h_opnd = function Reg r | Ind2 r | Ind3 (_, r) -> r_incr h r | Imm _ | Ind1 _ -> () in
...
let memop_of_prog (p : prog) : int =
  let count = ref 0 in
  let m_opnd = function Ind2 r | Ind3 (_, r) -> incr count | Reg _ | Imm _ | Ind1 _ -> () in
  let m_opcode = function
    ...
    | Retq ->
        ()
  in
  let h_ins (oc, ops) = m_opcode oc ; List.iter m_opnd ops in
  let h_elem {lbl; global; asm} =
    match asm with Text insns -> List.iter h_ins insns | Data _ -> ()
  in
  let _ = List.iter h_elem p in
...
let summary (h : histogram) : int = Array.fold_left ( + ) 0 h - (2 * h.(idx_of_reg Rbp))
