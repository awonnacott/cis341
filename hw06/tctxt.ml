...
(* bundled together *)
type t = {locals: local_ctxt; globals: global_ctxt; functions: fun_ctxt; structs: struct_ctxt}
...
let lookup_option id c : Ast.ty option =
  match lookup_local_option id c with None -> lookup_global_option id c | Some x -> Some x
...
let lookup_struct_option id c = try Some (lookup_struct id c) with Not_found -> None
...
let lookup_field_option st_name f_name c =
  ...
  in
  match lookup_struct_option st_name c with None -> None | Some x -> lookup_field_aux f_name x

let lookup_field st_name f_name c =
  match lookup_field_option st_name f_name c with
  | None ->
      failwith "StructCtxt.lookup_field: Not found"
  | Some x ->
      x
