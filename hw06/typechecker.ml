...
let typ_of_binop : Ast.binop -> Ast.ty * Ast.ty * Ast.ty = function
  ...
  | And | Or ->
      (TBool, TBool, TBool)
  | Eq | Neq ->
      failwith "typ_of_binop called on polymorphic == or !="
...
let rec typecheck_ty (l : 'a Ast.node) (tc : Tctxt.t) (t : Ast.ty) : unit =
  match t with TBool -> () | TInt -> () | TNullRef r | TRef r -> typecheck_ref l tc r
...
and typecheck_ret l tc (rt : Ast.ret_ty) : unit =
  match (rt : Ast.ret_ty) with RetVoid -> () | RetVal t -> typecheck_ty l tc t
...
let rec typecheck_exp (c : Tctxt.t) (e : Ast.exp node) : Ast.ty =
  ...
  | Id i -> (
    ...
    | Some x ->
        x
    | None ->
        type_error e ("Unbound identifier " ^ i) )
  | CArr (t, l) ->
      typecheck_ty e c t ;
  ...
  | NewArr (t, e1, id, e2) ->
      ...
      if size_type = TInt then
        ...
        if subtype c t' t then TRef (RArray t) else type_error e2 "Initializer has incorrect type"
  ...
  | Bop (b, l, r) -> (
      ...
      | _ ->
          ...
          if bl = ltyp then
            if br = rtyp then bres else type_error r "Incorrect type in binary expression"
  ...
  | Index (e1, e2) ->
      ...
      if ind_t = TInt then
        ...
        | TRef (RArray t) ->
            t
        | _ ->
            type_error e1 ("Tried to compute index into type " ^ Astlib.string_of_ty arr_t)
  ...
  | Proj (s, id) -> (
      ...
      | TRef (RStruct sn) -> (
        ...
        | None ->
            type_error e (id ^ " not member of struct " ^ sn)
        | Some t ->
            t )
      | _ ->
          type_error s "Cannot project from non-struct" )
  | CStruct (id, l) -> (
    match Tctxt.lookup_struct_option id c with
    | None ->
        type_error e (id ^ "not a struct type")
    | Some x ->
        ...
        if struct_names <> local_names then
          type_error e "Mismatch of fields between struct definition and local declaration" ;
        List.iter
          (fun (id, ft) ->
            let t = (List.find (fun i -> i.fieldName = id) x).ftyp in
            if not (subtype c ft t) then type_error e (id ^ " field of struct incorrect") else ()
            )
          field_types ;
        TRef (RStruct id) )
  | Length l -> (
      let t = typecheck_exp c l in
      match t with TRef (RArray t) -> TInt | _ -> type_error l "Cannot take length of non-array" )
  | Call (f, args) -> (
      ...
      | TRef (RFun (l, RetVal r)) ->
          if List.length l <> List.length argtyps then type_error e "Incorrect number of arguments"
            ...
            List.iter2
              (fun arg l -> if not (subtype c arg l) then type_error e "Incorrect type of argument")
              argtyps l ;
          r
      | _ ->
          type_error e "Need function argument for function call" )
...
let rec typecheck_stmt (tc : Tctxt.t) (s : Ast.stmt node) (to_ret : ret_ty) : Tctxt.t * bool =
  ...
  | Ret r -> (
    ...
    | Some r, RetVal to_ret ->
        ...
        if subtype tc t to_ret then (tc, true) else type_error s "Returned incorrect type"
    | None, RetVal to_ret ->
        type_error s "Returned void in non-void function"
    | Some r, RetVoid ->
        type_error s "Returned non-void in void function" )
  | SCall (f, args) -> (
      ...
      | TNullRef (RFun (l, RetVoid)) | TRef (RFun (l, RetVoid)) ->
          if List.length l <> List.length argtyps then type_error s "Incorrect number of arguments"
            ...
            List.iter2
              (fun arg l ->
                if not (subtype tc arg l) then type_error s "Incorrect type of argument" )
              argtyps l ;
          (tc, false)
      | _ ->
          type_error s "Need function argument for function call" )
  | If (e, b1, b2) ->
  ...
  | Cast (r, id, exp, b1, b2) -> (
      ...
      | _ ->
          type_error exp "if? expression has non-? type" )
  | While (b, bl) ->
