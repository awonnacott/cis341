...
let string_of_range (f, (sl, sc), (el, ec)) = Printf.sprintf "%s:[%d.%d-%d.%d]" f sl sc el ec
...
let pos_of_lexpos (p : position) : pos = mk_pos p.pos_lnum (p.pos_cnum - p.pos_bol)
...
let lex_range lexbuf : t = mk_lex_range (lexeme_start_p lexbuf) (lexeme_end_p lexbuf)
